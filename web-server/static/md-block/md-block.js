...
const URLs = {
    ...
    //md-block does not work with the latest version of marked, so this is a quick work around:
...
class MarkdownElement extends HTMLElement {
    ...
    async render() {
        ...
        if (!window.Prism && URLs.window.Prism && this.querySelector("code")) {
            window.Prism = import(URLs.window.Prism);

            if (URLs.window.PrismCSS) {
                let link = document.createElement("link");
                link.rel = "stylesheet";
                link.href = URLs.window.PrismCSS;
                document.head.appendChild(link);
            }
        }

        if (window.Prism) {
            await window.Prism; // in case it's still loading
